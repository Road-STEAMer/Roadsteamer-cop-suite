...
/*!************************************************!*\
  !*** ./app/packs/entrypoints/datasets_list.js ***!
  \************************************************/
document.addEventListener("DOMContentLoaded", function () {
  ...
  function closeModalIfEmpty(refreshPageFlag) {
    if (typeof counter !== 'undefined' && counter === 0) {
      // Assicurati che counter sia definito
      var modalOverlay = document.querySelector(".modal-overlay");
      if (modalOverlay) {
        modalOverlay.style.display = "none";
      }
  ...
  function deleteDataset(id) {
    ...
    if (confirm("Are you sure you want to delete this dataset?")) {
      fetch('/idra_delete', {
        ...
        body: JSON.stringify({
          selected_dataset_id: id
        })
      }).then(function (response) {
        if (response.ok) {
          updatePartialView(); // Assicurati che questa funzione sia definita
          counter--; // Assicurati che counter sia definito
          var deleteEvent = new CustomEvent('datasetDeleted', {
            detail: {
              deleted: deleted
            }
          });
...
/******/ })()
;
//# sourceMappingURL=datasets_list.js.map